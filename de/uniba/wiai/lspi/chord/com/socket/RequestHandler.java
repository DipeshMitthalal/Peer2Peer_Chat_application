...
final class RequestHandler extends Thread implements EndpointStateListener {
	...
	Serializable invokeMethod(int methodType, Serializable[] parameters)
		...
		case MethodConstants.SEND_MESSAGE: {
//			System.out.println(this.node.getNodeID() +" receive Send Message");
			((NodeImpl)this.node).receivedMessage((Entry) parameters[0]);
			break;
		}
		case MethodConstants.SEND_MESSAGE_NO_WAIT: {
//			System.out.println(this.node.getNodeID() +" receive No Wait Send Message");
			((NodeImpl)this.node).receivedMessageNoWait((Entry) parameters[0]);
			break;
		}
		case MethodConstants.DIRECT_SEND_BACK: {
			((NodeImpl)this.node).receivedMessageNoWait((Entry) parameters[0]);
			break;
		}
		default: {
