...
public final class SocketProxy extends Proxy implements Runnable {
	...
	public void sendEntry(Entry entryToSend) throws CommunicationException {
		this.makeSocketAvailable();

		logger.debug("Trying to send entry " + entryToSend + ".");

		/* prepare request for method insertEntry */
		Request request = this.createRequest(MethodConstants.SEND_MESSAGE,
				new Serializable[] { entryToSend });
		/* send request */
		try {
			logger.debug("Trying to send request " + request);
			this.send(request);
	...
	public void sendEntryNoWait(Entry entryToSend) throws CommunicationException {
		this.makeSocketAvailable();

		logger.debug("Trying to send entry " + entryToSend + ".");

		/* prepare request for method insertEntry */
		Request request = this.createRequest(MethodConstants.SEND_MESSAGE_NO_WAIT,
				new Serializable[] { entryToSend });
		/* send request */
		try {
			logger.debug("Trying to send request " + request);
