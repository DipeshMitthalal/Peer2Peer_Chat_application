...
final class Entries {
	 ...
	 */
	public final int getNumberRegUser(String keyword){
		int count=0;
		for (Map.Entry<ID, Set<Entry>> e : this.entries.entrySet()){
			if (e.getValue().toString().contains(keyword)) {
				count++;
			}
		}
		return count;
	}
	public final int getNumerValues(String keyword){
		int count = 0;
		for (Map.Entry<ID, Set<Entry>> e : this.entries.entrySet()){
			if (e.getValue().toString().contains(keyword)) {
