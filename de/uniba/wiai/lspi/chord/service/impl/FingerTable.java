...
final class FingerTable {
	 ...
	 */
	final int getFingerTableLength(){
		return this.remoteNodes.length;
	}
	final Node getRoutingUsableNode(ID key,int backoff){
		...
		for (int i = this.remoteNodes.length - (1+backoff); i >= 0; i--) {
			if (this.remoteNodes[i] != null
					&& this.remoteNodes[i].getNodeID().isInInterval(
							this.localID, key)) {
				if (debug) {
					this.logger.debug("Another (might be) usable node for ID " + key
		...
		if (debug) {
			this.logger.debug("There is no usable node for ID "
					+ key + " -- returning null!");
		}
		return null;
	}
	final Node isInFingerTable(ID key){
		Node result=null;
		for (int i= this.remoteNodes.length -1; i>=0;i--){
			if (this.remoteNodes[i].getNodeID().equals(key)){
				result = this.remoteNodes[i];
			}
		}
		return result;
	}
