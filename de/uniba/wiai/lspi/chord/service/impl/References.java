...
final class References {
	 ...
	 */
	final synchronized Node getClosetPreceedingNodeInFingerTable(ID key){
		Node node = this.fingerTable.getClosestPrecedingNode(key);
		return node;
	}
	final synchronized Node getRoutingUsableNodeInFingerTable(ID key,int backoff){
		Node node = this.fingerTable.getRoutingUsableNode(key, backoff);
		return node;
	}
	 ...
	 */
	final synchronized Node findNodeInRef(ID id){
		Node result = null;
		for (int i=0; i<this.fingerTable.getFingerTableLength(); i++){
			result = this.fingerTable.isInFingerTable(id);
		}
		return result;
	}
